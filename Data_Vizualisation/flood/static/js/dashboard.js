...
function aggregateMapData(minY, maxY) {
    ...
    filtered.forEach(d => {
        ...
        if (!grouped.has(zoneName)) {
            ...
            // Track by MAX Rainfall instead of Magnitude
            if (d.Rainfall_mm > existing.Rainfall_mm) {
                existing.Rainfall_mm = d.Rainfall_mm;
                existing.Flow_Velocity_mps = d.Flow_Velocity_mps;
                existing.Flood_Depth_Meters = d.Flood_Depth_Meters;
...
document.getElementById('btn-download').addEventListener('click', () => {
    ...
    const a = document.createElement('a'); a.setAttribute('href', url); a.setAttribute('download', 'resqfy_flood_export.csv'); a.click();
});
...
function init3DEngine() {
    ...
    renderer.localClippingEnabled = true; // Crucial for bounding the water
...
function render3DGeology(pointData) {
    let modelFile = 'mountain.glb'; 
    switch (pointData.Terrain) {
        case 'Mountain':   modelFile = 'mountain.glb'; break;
        case 'Urban':      modelFile = 'city.glb'; break;
        case 'Coastal_Plain': modelFile = 'river.glb'; break;
        case 'River_Valley':  modelFile = 'desert.glb'; break; // Reusing desert geometry for flat valley
    }

    // Clear old data layers instantly
    ...
    if (!glbDataUri) {
        console.warn(`WARNING: Data for ${modelFile} is missing in standalone injection.`);
        return;
    }
    ...
    // THE FIX: Instead of feeding the massive Base64 string directly to loader.load()
    // We convert it to a raw binary ArrayBuffer first, which stops ThreeJS from crashing.
    fetch(glbDataUri)
        ...
        .then(buffer => {
            loader.parse(buffer, '', function(gltf) {
                ...
                console.error("Error parsing the 3D model data:", err);
            });
        })
        .catch(err => {
            console.error("Failed to decode model data:", err);
            if (loaderOverlay) loaderOverlay.style.display = 'none';
        });
}

// ==========================================
// 3. THE LIVE DATA INJECTOR (FLOOD LOGIC)
// ==========================================
...
function injectDataLayers(pointData) {
    ...
    const width = blockBoundingBox.max.x - blockBoundingBox.min.x;
    const depthZ = blockBoundingBox.max.z - blockBoundingBox.min.z;
    const centerX = (blockBoundingBox.max.x + blockBoundingBox.min.x) / 2;
    const centerZ = (blockBoundingBox.max.z + blockBoundingBox.min.z) / 2;
    
    // Strict bounding planes so water doesn't bleed out of the block
    clippingPlanes = [
        ...
        new THREE.Plane(new THREE.Vector3(0, -1, 0), blockBoundingBox.max.y + 10), // Allow rain high up
    ...
    // HUD 2: Flood Telemetry
    let epiHTML = formatRow('Rainfall', pointData.Rainfall_mm, 'mm', 'alert');
    epiHTML += formatRow('Flow Velocity', pointData.Flow_Velocity_mps, 'm/s');
    epiHTML += formatRow('Flood Depth', pointData.Flood_Depth_Meters, 'm', 'alert');
    epiHTML += formatRow('Permeability', pointData.Soil_Permeability);
    ...
    // Track surface levels for the SVG lines
    trackPoints.surfTarget = new THREE.Vector3(0, blockBoundingBox.max.y, 0);
    controls.target.set(0, blockBoundingBox.max.y - (totalHeight/4), 0);
    controls.update();

    if (pointData.Flood_Depth_Meters) {
        // --- 1. RISING WATER PLANE ---
        const MAX_DEPTH_M = 15.0; // Assume 15 meters is visually the top of the "danger zone"
        let depthRatio = Math.min(pointData.Flood_Depth_Meters / MAX_DEPTH_M, 1.0);
        
        // Visually scale the flood depth so it looks good relative to the block's height (max 25% of block height)
        const maxVisualFloodHeight = totalHeight * 0.25; 
        const waterY = blockBoundingBox.max.y + (depthRatio * maxVisualFloodHeight);
        
        const waterGeo = new THREE.PlaneGeometry(width * 1.05, depthZ * 1.05); // slightly larger to prevent z-fighting on edges
        const waterMat = new THREE.MeshBasicMaterial({ 
            color: 0x0088ff, 
            transparent: true, 
            opacity: 0.65, 
            side: THREE.DoubleSide,
            clippingPlanes: clippingPlanes // Cut off at block edges
        });
        
        waterMesh = new THREE.Mesh(waterGeo, waterMat);
        waterMesh.rotation.x = -Math.PI / 2;
        waterMesh.position.set(centerX, waterY, centerZ);
        waterMesh.name = 'data-layer';
        scene.add(waterMesh);

        // Track the water level for the UI line
        trackPoints.epiTarget = new THREE.Vector3(0, waterY, 0); 

        // --- 2. PARTICLE RAIN SYSTEM ---
        const baseDropCount = 1000;
        // Scale rain density by Rainfall_mm
        const dropCount = Math.floor(baseDropCount * (pointData.Rainfall_mm / 100.0));
        
        rainGeo = new THREE.BufferGeometry();
        const rainPos = new Float32Array(dropCount * 3);
        
        for(let i=0; i<dropCount; i++) {
            rainPos[i*3] = blockBoundingBox.min.x + Math.random() * width;
            rainPos[i*3+1] = waterY + Math.random() * 12; // Start falling from 12 units above water
            rainPos[i*3+2] = blockBoundingBox.min.z + Math.random() * depthZ;
        }
...
function animate3D() {
    ...
    // Animate Rain
    if (rainGeo && waterMesh && blockBoundingBox) {
        const positions = rainGeo.attributes.position.array;
        for(let i=1; i<positions.length; i+=3) {
            positions[i] -= 0.35; // Fall speed
            // Reset drop to the top if it hits the water surface
            if (positions[i] < waterMesh.position.y) {
                positions[i] = waterMesh.position.y + 10 + Math.random() * 4;
            }
        }
        rainGeo.attributes.position.needsUpdate = true;
    }
    
    // Bobbing water effect
    if (waterMesh) {
        waterMesh.position.y += Math.sin(Date.now() * 0.002) * 0.005;
    }
...
function loadSector(aggZoneData) {
    ...
    document.getElementById('ui-subtext').textContent = "Sector Render Active // Flood Scan Mode";
...
function generateMap(minYear, maxYear) {
    ...
    // Scale map bubbles by Rainfall
    const impactSizes = mapData.map(d => Math.pow(d.Rainfall_mm, 1.1) / 5);
    const maxSize = Math.max(...impactSizes); const minSize = Math.min(...impactSizes);
    const normalizedSizes = impactSizes.map(s => 10 + ((s - minSize) / (maxSize - minSize || 1)) * 30);
    ...
    const trace = {
        ...
        marker: { 
            ...
            color: mapData.map(d => d.Rainfall_mm), 
            colorscale: configParams.color_scale, 
            showscale: true, 
            opacity: 0.9, 
            line: { color: '#000', width: 1.5 } 
        },
        text: mapData.map(d => d.ZoneName + `<br>Rainfall: ${d.Rainfall_mm} mm<br>Depth: ${d.Flood_Depth_Meters} m`), 
...
function generateTrendGraph(zoneName) {
    ...
    for (let year = 2007; year <= 2017; year++) {
        ...
        stats[year] = yearData.length > 0 ? yearData.reduce((sum, d) => sum + d.Rainfall_mm, 0) / yearData.length : null;
    }
    const years = Object.keys(stats).map(Number); const vals = years.map(y => stats[y]);
    const trace = { 
        x: years, y: vals, 
        name: 'Rainfall', type: 'scatter', mode: 'lines+markers', 
        line: { color: '#0088ff', width: 2, shape: 'spline' }, 
        fill: 'tozeroy', fillcolor: 'rgba(0, 136, 255, 0.1)',
        connectgaps: true 
    };
    return { data: [trace], layout: { title: { text: `RAINFALL HISTORY: ${zoneName.toUpperCase()}`, font: { color: '#00d4ff', size: 12, family: 'monospace' } }, paper_bgcolor: '#080808', plot_bgcolor: '#080808', font: { color: '#888', size: 10 }, margin: { l: 35, r: 20, t: 35, b: 20 }, yaxis: { gridcolor: '#1a1a1a', title: 'Rainfall (mm)' }, xaxis: { gridcolor: '#1a1a1a' } } };
}
...
function executeLiveSimulation() {
    ...
    const simMag = parseFloat(document.getElementById('test-mag').value); // Rainfall
    const simDepth = parseFloat(document.getElementById('test-depth').value);
    const simPGA = parseFloat(document.getElementById('test-pga').value); // Velocity
    ...
    const mockData = {
        Terrain: simTerrain,
        Rainfall_mm: simMag,
        Flood_Depth_Meters: simDepth,
        Flow_Velocity_mps: simPGA,
        Affected_Population: simPop,
        Economic_Loss_Millions: simLoss,
        Block_ID: "SYS_OVERRIDE_MANUAL",
        Soil_Permeability: "Synthetic Test Base",
        Lat: "0.00",
        Lon: "0.00"
    };
    ...
    document.getElementById('sector-timeline').innerHTML = `<div class="timeline-label" style="font-size:13px; color:#0088ff; font-weight:700; letter-spacing: 2px;">LIVE SIMULATION ENGAGED</div>`;
